...
cgen_batching_u280_cp_envelop = np.nanmax([cgen_b1_u280_datcopy, cgen_b10_u280_datcopy, cgen_b50_u280_datcopy], axis=0)


cgen_batching_vck5000_cp_envelop = np.nanmax([cgen_b1_vck5000_datcopy, cgen_b10_vck5000_datcopy, cgen_b50_vck5000_datcopy], axis=0)


u280_cp_envelop_source = np.nanargmax([cgen_b1_u280_datcopy, cgen_b10_u280_datcopy, cgen_b50_u280_datcopy], axis=0)

vck5000_cp_envelop_source = np.nanargmax([cgen_b1_vck5000_datcopy, cgen_b10_vck5000_datcopy, cgen_b50_vck5000_datcopy], axis=0)
...
geometric_means = {
    ...
    'U280 without batching': geometric_mean(cgen_b1_u280_datcopy),
    'VCK5000 without batching': geometric_mean(cgen_b1_vck5000_datcopy),
    'H100 100B': geometric_mean(h100_50b),
}
...
maximum_throughputs = {
    ...
    'U280 without batching': np.nanmax(cgen_b1_u280_datcopy),
    'VCK5000 without batching': np.nanmax(cgen_b1_vck5000_datcopy),
    'H100 50B': np.nanmax(h100_50b),
}
...
batch_sizes = np.array(['1B', '10B', '50B'])
...
u280_cp_bars = ax.bar(x_indexes - bar_width + bar_offset, cgen_batching_u280_cp_envelop, width=bar_width, label='CP_50B_U280', color=colors[5])
...
bar3 = ax.bar(x_indexes - bar_width + bar_offset, cgen_b1_u280_datcopy, width=bar_width, label='CP_WOB_U280', color='white', hatch='xx')
bar3 = ax.bar(x_indexes - bar_width + bar_offset, cgen_b1_u280_datcopy, width=bar_width, color='none', edgecolor='black', **iner_props)
...
vck5000_cp_bars = ax.bar(x_indexes + bar_offset, cgen_batching_vck5000_cp_envelop, width=bar_width, label='CP_50B_VCK5', color=colors[1])
...
plt.rcParams['hatch.color'] = colors[1]
bar6 = ax.bar(x_indexes + bar_offset, cgen_b1_vck5000_datcopy, width=bar_width, label='CP_WOB_VCK5', color='white', hatch='xx')
bar6 = ax.bar(x_indexes + bar_offset, cgen_b1_vck5000_datcopy, width=bar_width, color='none', edgecolor='black', **iner_props)
...
# Save the figure
fig.tight_layout()
plt.savefig("output/jacobi3d_batched_throughput_barplot.pdf", bbox_inches='tight')
# plt.show()
