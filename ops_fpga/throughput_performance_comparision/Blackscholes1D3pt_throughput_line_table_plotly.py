...
cgen_u280_power = df["pow_C_U280_10000B"]
h100_power = df["pow_H100_10000B"]
...
table_data = [
    [f"{xticks[i]}", f"{cgen_u280_power[i]:.2f}", f"{h100_power[i]:.2f}"] for i in range(4,len(xticks))
]
column_labels = ["","U280", "H100"]
row_labels = [f"{xtick}" for xtick in xticks[4:]]
...
fig.add_trace(go.Table(
    ...
    domain=dict(x=[0.79, 1.0], y=[0, 0.8])  # Position the table to the right of the plot
))

fig.update_layout(
    annotations=[
        dict(
            text="Power-10000B (KJ)",  # Title text
            x=1.0,  # Center the title horizontally
...
fig.write_image("output/blackscholes1d3pt_throughput_lineplot_with_table_plotly.pdf")