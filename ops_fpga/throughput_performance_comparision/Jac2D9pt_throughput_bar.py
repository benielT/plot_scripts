...
bar_width = 0.10  # Adjust as needed
x_indexes = np.arange(len(xticks))

props = {'linestyle':'--', 'linewidth':'1.0'}
# Plot throughput as bars
bar1 = ax.bar(x_indexes - 2 * bar_width, hand_u280, width=bar_width, hatch='//', label='h_U280', color='white')
bar1 = ax.bar(x_indexes - 2 * bar_width, hand_u280, width=bar_width, color='none', edgecolor='black', **props)
bar2 = ax.bar(x_indexes - bar_width, cgen_4096_u280, width=bar_width, label='c_U280_4096', color=colors[1])
bar3 = ax.bar(x_indexes, cgen_8192_u280, width=bar_width, label='c_U280_8192', color=colors[6])
plt.rcParams['hatch.color'] = colors[2]
